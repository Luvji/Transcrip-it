...
On Sep 21, 2026, network-system-call traces covered a real microphone capture plus the exact local FFmpeg preparation and installed `whisper.cpp` inference executables. Capture opened only local `AF_UNIX` connections to `/run/user/1001/pulse/native`; FFmpeg and Whisper made no network connection calls, and Whisper produced its local JSON output. No `AF_INET` or `AF_INET6` socket appeared in any trace.
